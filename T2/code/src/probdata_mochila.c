   ...
   )
{
   ...
   SCIP_CALL( SCIPallocBufferArray(scip, &vars, I->n+I->m) ); // aloca memoria p
   //   SCIP_CALL( SCIPallocBufferArray(scip, &conss, 1) );
   //   SCIP_CALL( SCIPallocBufferArray(scip, &vars, I->n) ); // aloca memoria para as variaveis xi e yj, para cada item i e para cada elemento j
   ...
   for( i = 0; i < I->n; nvars++, ++i )
   {
      ...
      if(!fixed){
         ...
         lbvar = fixed[i]==1?1.0:0.0;
         ubvar = fixed[i]==-1?0.0:1.0;
      }
