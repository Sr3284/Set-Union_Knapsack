...
int aleatoria(SCIP* scip, SCIP_SOL** sol, SCIP_HEUR* heur)
{
   ...
   unsigned int stored;
   int nvars;
   int *covered, n, custo, nCovered, *cand, nCands, selected, s;
   ...
   for(i=0;i<n && !infeasible && nCands > 0 && residual>0;i++){
      s = randomIntegerB (0, nCands-1); // rand
      selected = cand[s]; // selected candidate
      cand[s] = cand[--nCands]; // remove selected candidate
     // only select actived var in scip and whose aleatoria up is valid for the problem
      peso = 0;
